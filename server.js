...
const upload = multer({ 
    ...
    fileFilter: (req, file, cb) => {
        ...
        if (allowedMimeTypes[file.mimetype]) {
            ...
            // 提供更友好的错误信息
            cb(new Error(`不支持的文件类型: ${file.mimetype}。请上传Word文档、PDF或图片文件。`), false);
        }
    },
    // 增加文件大小限制到100MB
    limits: {
...
app.get('/api/download/:id', (req, res) => {
    ...
    db.getFileById(fileId, (err, file) => {
        ...
        if (!fs.existsSync(file.filepath)) {
            return res.status(404).json({ success: false, message: '文件已丢失。注意：Render是无状态平台，重启或重新部署后文件会自动清理。建议使用持久化存储服务。' });
        }
